...
try:
    gatewayOptions = {"org": "4vwckx", "type": "raspberryGW", "id": "gateway1", "auth-method": "token", "auth-token": "polarbear"}
    gatewayCli = ibmiotf.gateway.Client(gatewayOptions)

    gatewayCli.connect()
    gatewayCli.deviceCommandCallback = myCommandCallback 
    gatewayCli.subscribeToDeviceCommands(deviceType='raspberryGW', deviceId='gateway1', command='display',format='json',qos=2)

    while True:
        temp = sense.get_temperature() # TODO insert your code here
        myData = {"d" : {"temperature" : temp}} # TODO insert your code here
        gatewayCli.publishDeviceEvent("senseHat", "sengateway1", "yourEventName", "json", myData, qos=1 )
        time.sleep(2)
